...
class Storywrangler:
    ...
    def get_zipf_dist(self,
        ...
        try:
            ngram_order = int(re.findall(r'\d+', ngrams)[0])
        except IndexError:
            logger.warning(f"Could not infer ngram order from collection {ngrams}. Defaulting to n = 1.")
            ngram_order = 1
